...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${base}/export/csv?${params.toString()}`, {
    headers: {
  ...
  return new NextResponse(await response.text(), {
    headers: {
      ...
      'content-disposition': response.headers.get('content-disposition') ?? 'attachment; filename=preisverlauf.csv',
    },
  })
}
